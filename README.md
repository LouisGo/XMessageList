...
公开集成模型为：应用级 session registry + 按 feed 的 session + React 投射适配器。应用通过适配器一次性配置 `request`、`row`、`anchorMemory` 和 `readReceipts` 行为；React 负责渲染已有的 `MessageListSession`。
...
- `src/x-message-list/core/session-registry`：应用编排层。按 session/feed id 懒创建 `MessageListSession`，路由适配器，掌管请求桥接、keepAlive 保留、`anchorMemory` 和 `readReceipts`。
- `src/x-message-list/core/runtime`：框架无关的视口 runtime 与内部 data runtime。消费已合并的 loaded segment，序列化投射事务，掌管 DOM ref/测量，写入 `scrollTop`，保持视觉锚点，发出 need 事件并报告诊断/evidence。
...
- 包根不导出 `createMessageListRuntime`、`MessageListRuntime`、`MessageListSnapshot`、`MessageListRuntimeEvent`、`LoadedSegment`、`MessageDataItem`、data runtime 类型或 `x-message-list/data` 子路径。
- `MessageListSessionRegistry` 掌管所有 feed 关联的 session。`MessageList` 卸载时仅分离视图，不会销毁 session。
- 应用 store 仍然是分页消息缓存、持久化和脏时间戳检查的规范持有者。XMessageList session 仅掌管当前 loaded segment、边缘状态、视口状态和已加载行的变更。
- `MessageListSession` 仅暴露公开应用命令和本地行变更入口：`commands.scrollToLatest`、`commands.scrollToMessage`、`commands.loadBefore`、`commands.loadAfter`、`commands.reloadLatest`、`getState`、`subscribe`、`rows.patch`、`rows.mutate`、`rows.replace`、`rows.resetLatest`、`rows.resetAround`、`rows.applyIdentityRemap` 和 `rows.clear`。
- React 是 `MessageListSession` 之上的适配器；它不得调用 request API、合并数据、持久化锚点或执行已读回执。
- Runtime 和 data runtime 保持为包内部实现细节。
