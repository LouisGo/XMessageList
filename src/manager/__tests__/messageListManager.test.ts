...
describe('createMessageListManager', () => {
  it('lazily creates, routes, and reuses a warm session controller', async () => {
    const normalAdapter = createAdapter('normal')
    const favoriteAdapter = createAdapter('favorite')
    const resolveAdapter = vi.fn((conversation: TestConversation) =>
    ...
    const controller = manager.getController('favorite-1')
    const sameController = manager.getController('favorite-1')
    ...
    await waitFor(() => controller.getSnapshot().items.length === 1)
    expect(controller.getSnapshot().items[0].message?.id)
  ...
  it('evicts inactive overflow sessions by keepAlive policy', () => {
    const manager = createMessageListManager<TestRow, TestConversation>({
      ...
      resolveConversation: (id) => ({ id, type: 'normal' }),
      resolveAdapter: () => createAdapter('normal'),
    })

    manager.getController('feed-a')
    manager.getController('feed-b')
  ...
  it('destroys sessions only through manager policy or explicit calls', async () => {
    ...
    const controller = manager.getController('feed-a')
  ...
  it('restores around a memory anchor before falling back to latest', async () => {
    ...
    const manager = createMessageListManager<TestRow, TestConversation>({
      ...
      resolveAdapter: () => ({
        ...
        memory: {
          loadAnchor: () => ({ id: 'restored' }),
          saveAnchor: () => undefined,
        },
      }),
    })
    const controller = manager.getController('feed-a')
    ...
    expect(controller.getSnapshot().items[0].message?.id).toBe('restored')
  })
  ...
  it('rejects stale bootstrap/reload responses without overwriting newer rows', async () => {
    ...
    await waitFor(() => controller.getSnapshot().items[0]?.message?.id === 'new')

    pending[0](page(['old']))
    await wait()

    expect(controller.getSnapshot().items[0]?.message?.id).toBe('new')
    expect(requestResults).toEqual(['latest:applied', 'latest:stale'])
  })
})

describe('MessageListReadReceiptWorker', () => {
  it('batches visible rows and dedupes already sent keys', async () => {
    const markRead = vi.fn((rows: TestRow[]) => {
      void rows
      return Promise.resolve()
    })
    const worker = new MessageListReadReceiptWorker(
      createAdapter('normal', {
        readReceipt: {
          batchDelayMs: 0,
...
function createAdapter(
  ...
  overrides: Partial<MessageListAdapter<TestRow, TestConversation>['request']> & {
    readReceipt?: MessageListAdapter<TestRow, TestConversation>['readReceipt']
  } = {},
  ...
  return {
    ...
    readReceipt: overrides.readReceipt,
  }
}
