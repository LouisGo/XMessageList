...
- 数据变化必须先带上滚动语义，例如 `prepend`、`append`、`auto-scroll-to-bottom`。
...
type MotionDecision =
  ...
  | {
      ...
      source: Extract<ScrollSource, 'programmatic' | 'followBottom'>;
      targetTop: number;
...
3. 如果距离大于 `maxDistancePx`：
   ...
   - 这一步仍标记为 runtime programmatic write。
4. 用 rAF 动画剩余距离。
...
2. 让 runtime 在 `cancelScheduledWork()` / reset / detach / destroy 中取消 motion。
3. 接入 `followBottom`，只覆盖 `hasMoreAfter=false` 的 latest target。
4. 接入 bottom locked append / auto-scroll-to-bottom。
5. 接入 `jump`，并确保 target row commit + measure 后才启动。
6. 加用户输入取消。
7. 扩展测试到 far distance 和 reduced motion。
8. 再评估是否实现 delete/collapse 的 preserve-anchor animation loop。
