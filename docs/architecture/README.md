...
| [message-runtime-implementation-contract.md](../viewport-runtime/message-runtime-implementation-contract.md) | 已有 | 需要从零实现 runtime、定义 API、command、transaction、React commit 回执和测试合同前 | public API, transaction, command queue |
| [message-list-transition-boundary.md](../typex-transition/message-list-transition-boundary.md) | 已有 | 需要处理迁移期 `@messageList` 与新架构共存、替换和隔离策略时 | migration boundary, legacy adapter, compatibility surface |
| [viewport-runtime/README.md](../viewport-runtime/README.md) | 新增 | 需要具体实现 IM 滚动视图容器 runtime、React adapter、RenderWindow、spacer、transaction、teardown 和测试时 | viewport runtime implementation, React external store, DOM registry |
...
6. 从零实现 runtime：读 `../viewport-runtime/message-runtime-implementation-contract.md`。
7. 具体实现滚动视图容器：读 `../viewport-runtime/README.md` 及其子文档。
8. 涉及现有 `@messageList`：最后读 `../typex-transition/message-list-transition-boundary.md`。
...
- `viewport-runtime/*`：专注 renderer 内部滚动视图容器 runtime 的实现施工图。
